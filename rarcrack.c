...
const char* CMD[] = {
    "unrar t -y -p%s %s 2>&1", "7z t -y -p%s %s 2>&1", "unzip -P%s -t %s 2>&1", ""};
...
char password_good[PWD_LEN + 1] = {
    '\0', '\0'};              // this changed only once, when we found the good passord
...
char         finalcmd[300] = {
            '\0', '\0'};   // this depending on arhive file type, it's a command to test file with password
            ...
            for (node = root->children; node; node = node->next) {
                if (xmlStrcmp(node->name, (const xmlChar*)"current") == 0) {
                    ...
                    xmlFree(tmp);
                           ...
                           (xmlStrcmp(node->name, (const xmlChar*)"good_password") == 0)) {
...
int loadstatus() {
    ...
    if (root) {
            ...
            if (xmlStrcmp(node->name, (const xmlChar*)"abc") == 0) {
                if (node->children && (strlen((const char*)node->children->content) > 0)) {
                    ABC = (char*)xmlStringDecodeEntities(parserctxt,
                                                         (const xmlChar*)node->children->content,
                                                         XML_SUBSTITUTE_BOTH,
                                                         0,
                                                         0,
                                                         0);
                ...
                if (node->children && (strlen((const char*)node->children->content) > 0)) {
                    tmp = (char*)xmlStringDecodeEntities(parserctxt,
                                                         (const xmlChar*)node->children->content,
                                                         XML_SUBSTITUTE_BOTH,
                                                         0,
                                                         0,
                ...
                if (node->children && (strlen((const char*)node->children->content) > 0)) {
                    tmp = (char*)xmlStringDecodeEntities(
                        parserctxt, node->children->content, XML_SUBSTITUTE_BOTH, 0, 0, 0);
                    strcpy(password, tmp);
                    curr_len = strlen(password);
                    xmlMutexLock(finishedMutex);
                    finished = 1;
        ...
        node = xmlNewTextChild(
            root, NULL, (const xmlChar*)"current", (const xmlChar*)getfirstpassword());
...
void* status_thread() {
    ...
    while (1) {
        ...
        xmlMutexUnlock(pwdMutex);
        savestatus();   // FIXME: this is wrong, when probing current password(s) is(are) not
                        // finished yet, and the program is exiting
    }
}
    ...
    if (argc == 1) {
        ...
        for (i = 1; i < argc; i++) {
            if (strcmp(argv[i], "--help") == 0) {
                printf("Usage:   rarcrack encrypted_archive.ext [--threads NUM] [--type "
                       "rar|zip|7z]\n\n");
...
int main(int argc, char** argv) {
    // Print author
    printf("RarCrack! Forked from 0.2 by David Zoltan Kedves (kedazo@gmail.com)\n");
    printf("Modified Version 0.0 by LeMonMon\n\n");
